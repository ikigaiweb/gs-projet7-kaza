...
export default function Dropdown(props) {                  // creation d'une fonction auquel on cree le lien props avec son parent (About.js)
        ...
        <div className="large-Dropdown">
                ...
                <img                                       // Petite fleche auquel j'ai rajouté une annimation permettant del'inver lors dur click
                    ...
                    className={
                        toggle                             // on a clické ? non (false) donc rien se passe . On a clické la fleche s'anime
                            ? "large-Dropdown-visible img" // Class (css) qui fait reference à ma fleche (descendante)
                            : "large-Dropdown-visible img dropdown-arrow-animated"  // Class (css) qui fait reference à ma fleche (180°)
                    }
                />
            </div>
            ...
            <div
                ...
                className={                                // lors du clique la class (css) 'dropdown-animated est activé'
                    toggle ? "large-Dropdown-toggle dropdown-animated" : "large-Dropdown-toggle mini-Dropdown-toggle-box-hidden "
                }                                         //
            ...
            >
                <div className="large-Dropdown-toggle-p" aria-hidden={toggle ? "true" : "false"}>
                    {props.content}
                </div>
            </div>
        </div>                                             //Juste audessus me permet d'inserer mon paragraphe (props.content) qui sera lie à son parent (about.js)
    );
}
